...
class CargoPlaceListClient:
    ...
    def generate_cargo_place(
        ...
        """Генерирует один объект грузоместа со всеми обязательными полями."""
        dims = self._generate_random_dimensions()
        time_windows = self._generate_random_datetime_window()

        return {
            "status": "waiting_for_sending",  # ОБЯЗАТЕЛЬНОЕ
            "barCode": bar_code,
            "type": random.choice(self.CARGO_TYPES),
            # УБРАНО: departureAddress, deliveryAddress
            ...
            **dims,
            **time_windows,
            # Опциональные поля (без ИНН/КПП!)
            "wmsNumber": f"WMS-{external_id}",
            "invoiceDate": "2025-03-14",
            # УБРАНО: innShipper, kppShipper, innConsignee, kppConsignee
        }
